...
ADD files-to-upload files-to-upload
ADD local-files local-files

ARG USER_NAME="eric-oss-file-notification-enm-stub"
RUN echo "$USER_ID:x:$USER_ID:0:An Identity for $USER_NAME:/nonexistent:/bin/false" >>/etc/passwd

ARG JAR_FILE
ADD target/${JAR_FILE} eric-oss-file-notification-enm-stub-app.jar
...
-Dcom.sun.management.jmxremote.access.file=/jmx/jmxremote.access -jar eric-oss-file-notification-enm-stub-app.jar
...
LABEL \
    org.opencontainers.image.title=eric-oss-file-notification-enm-stub-jsb \
